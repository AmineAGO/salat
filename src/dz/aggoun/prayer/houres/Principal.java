...
public class Principal extends Frame implements Runnable {
	...
	private int  joumouaM=0,deltaHijir=0,changementHeure=0;
...
if(type!=null)
	...
	input = new FileInputStream("config_tmp.properties");
			...
			// get the property value and print it out
		 	...
		 	changementHeure= Integer.valueOf(prop.getProperty("changementHeure").replace(" ", ""));
		...
		System.out.println("Lancement du: "+now);
				...
				DateHeure = new HeurePanel().mettreEnForme(hEcran,heureFond,couleurPoliceHeure,joumoua,deltaHijir,changementHeure);
			...
			if (salatNow != null) {
				...
				try {
					...
					{	miseEnFormePanelPrieres(true);									
						...
						miseEnFormePanelPrieres(false);//ne pas afficher
					}
					contentPane.add(panelPrieres, BorderLayout.CENTER);
					contentPane.add(DateHeure, BorderLayout.PAGE_END);
					contentPane.repaint();
				...
				try {
					Thread.sleep((salatNow.getAttente_p() - 3) * 60000 + 40000);	//sleep le temps d'attente ikama
					contentPane.remove(panelPrieres);
					
					if(notDorhJoumou3a())
					{  
						contentPane.remove(panelIkama);//n'est pas affiché
					}  
	...
	void miseEnFormePanelPrieres(boolean ikama) {
		...
		panelPrieres1 = new PrierePanel().mettreEnForme(icha.getMb(),icha.getPriereCouleur(), icha.getColeur(), icha.getNom(),icha.getHeure_p() + ":" + completerZero(icha.getMin_p()),icha.getFond(),null,changementHeure);
		panelPrieres2 = new PrierePanel().mettreEnForme(maghreb.getMb(),maghreb.getPriereCouleur(), maghreb.getColeur(), maghreb.getNom(),	maghreb.getHeure_p() + ":" + completerZero(maghreb.getMin_p()),	maghreb.getFond(),null,changementHeure);
		panelPrieres3 = new PrierePanel().mettreEnForme(asr.getMb(),asr.getPriereCouleur(), asr.getColeur(),asr.getNom() ,asr.getHeure_p() + ":" + completerZero(asr.getMin_p()),asr.getFond(),null,changementHeure);
		panelPrieres4 = new PrierePanel().mettreEnForme(dohr.getMb(),dohr.getPriereCouleur(), dohr.getColeur(), dohr.getNom(),dohr.getHeure_p() + ":" + completerZero(dohr.getMin_p()),dohr.getFond(),null,changementHeure);
		panelPrieres5 = new PrierePanel().mettreEnForme(sobh.getMb(),sobh.getPriereCouleur(), sobh.getColeur(), sobh.getNom(),"0" + sobh.getHeure_p() + ":" + completerZero(sobh.getMin_p()),sobh.getFond(),"0" + chourouk.getHeure_p() + ":" + completerZero(chourouk.getMin_p()),changementHeure);
		
	 
		if(salatNow!=null && ikama){	
			panelIkama = new IkamaPanelPetite().mettreEnForme(image_ikama,salatNow.getAttente_p() - 2,hEcran,salatNow.getNom() , sonIkama,changementHeure); 
			if(salatNow==sobh){
	...
	void extraireInfosJour() throws IOException {
		...
		jour = Jour.mapToJour(fichierDuJour.getSearchLine(calendar.get(Calendar.DAY_OF_MONTH) - 1), image_off, fond_off, white,	priereCouleur_off,changementHeure);
